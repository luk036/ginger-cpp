...
TEST_CASE("Vector2 Construction") {
    ...
    SUBCASE("Copy construction") {
        ...
        CHECK_EQ(v2.x(), doctest::Approx(5.0));
        CHECK_EQ(v2.y(), doctest::Approx(10.0));
    }
}
...
TEST_CASE("Vector2 Operations") {
    ...
    SUBCASE("Dot product") {
        CHECK_EQ(v1.dot(v2), doctest::Approx(3 * 5 + 4 * 6));
        CHECK_EQ(v1.dot(v3), doctest::Approx(3 * 1.5 + 4 * 2.5));
    }

    SUBCASE("Cross product") {
        CHECK_EQ(v1.cross(v2), doctest::Approx(3 * 6 - 4 * 5));
        CHECK_EQ(v1.cross(v3), doctest::Approx(3 * 2.5 - 1.5 * 4));
    }
...
TEST_CASE("Vector2 Arithmetic Operators") {
    ...
    SUBCASE("Scalar division") {
        ...
        auto v4 = v / 3.0;
        CHECK_EQ(v4.x(), doctest::Approx(2.0));
        CHECK_EQ(v4.y(), doctest::Approx(3.0));
...
//         auto v = vi + vd;
//         CHECK_EQ(v.x(), doctest::Approx(4.5));
//         CHECK_EQ(v.y(), doctest::Approx(6.5));
//     }

//     SUBCASE("Mixed type subtraction") {
//         auto v = vd - vi;
//         CHECK_EQ(v.x(), doctest::Approx(-1.5));
//         CHECK_EQ(v.y(), doctest::Approx(-1.5));
//     }
