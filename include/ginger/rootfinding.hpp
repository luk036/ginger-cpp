...
/**
 * @brief Single-threading Bairstow's method (even degree only)
 *
 * The `pbairstow_even_st` function implements Bairstow's method for finding the roots of a real
 * polynomial with an even degree using a single thread.
 *
 * Each iterate handles one quadratic factor @f$ x^2 - r_i x - q_i @f$, applying:
 * @f[
 ...
 *
 * @return The function `pbairstow_even_st` returns a `std::pair<unsigned int, bool>`. The first
