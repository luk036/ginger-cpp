...
To collect code coverage information, run CMake with the `-DGINGER_ENABLE_COVERAGE=1` option.
...
```bash
cmake -B build -DGINGER_BUILD_DOCS=ON
...
# run standalone
./build/Ginger --help
# build docs (requires -DGINGER_BUILD_DOCS=ON at configure time)
cmake --build build --target GenerateDocs
```
...
clang-tidy can be enabled by configuring CMake with `-DGINGER_ENABLE_CLANG_TIDY=ON` and building the `clang-tidy` target.
